...
export function JobSiteStatusBar(props: { job: Job; onUpdate: (next: Job) => void }) {
  ...
  /**
   * ステータス変更（媒体の状態が変わった扱い）
   * - updatedAt: 更新（=媒体更新日）
   * - rpoLastTouchedAt: 更新
   * - ★「この端末で触った」印を残す
   * - ★イベントログを積む（STATUS_CHANGE）
   */
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <div className="text-sm font-semibold text-slate-900">媒体ステータス</div>
            {summary ? (
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 md:grid-cols-2">
            {allSites.map((site) => {
              ...
              return (
                <label
                  key={site}
                  className="flex items-start gap-2 rounded-xl border bg-white px-3 py-2 text-sm text-slate-800"
                  style={{ borderColor: "var(--border)" }}
                >
                  <input
                    type="checkbox"
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="ml-2 text-xs text-slate-500">{site}</span>
                  </span>
                </label>
              );
            })}
            ...
            return (
              <div key={siteKey} className="relative">
                <button
                  type="button"
                  className={[
                    "group inline-flex items-center gap-2 rounded-2xl border bg-white px-3 py-2 shadow-sm",
                    "hover:bg-slate-50",
                  ].join(" ")}
                  style={{ borderColor: "var(--border)" }}
                  onClick={() => setOpenKey((k) => (k === siteKey ? null : siteKey))}
                ...
                >
                  ...
                  {st.updatedAt ? (
                    ...
                    <span className="hidden text-[11px] text-slate-400 md:inline">未更新</span>
                  )}
                </button>
                ...
                {isOpen ? (
                  <div
                    className="absolute left-0 top-[calc(100%+10px)] z-30 w-[360px] rounded-2xl border bg-white p-4 shadow-xl"
                    style={{ borderColor: "var(--border)" }}
                  >
                    ...
                    <div className="mt-4">
                      <div className="text-xs font-semibold text-slate-800">ステータス</div>
                    ...
                    <div className="mt-4">
                      <div className="text-xs font-semibold text-slate-800">メモ（任意）</div>
                      <textarea
